...
// ── HARDCODED CONFIG ──────────────────────────────────────────────────────────
const IMALI_API_KEY = "sk_live_40825ffa8b0595ae3091d60c63ae30dc90a9e6af";
...
async function imali(method, path, body) {
  const res = await fetch(`${IMALI_BASE_URL}${path}`, {
    method,
    headers: {
  ...
  if (!res.ok) {
    const err = new Error(data?.error?.message ?? "iMali API error");
